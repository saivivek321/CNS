...
string encrypt(string pt, vector<string> rkb) {
	...
	cout<<"Left Plain Text : "<<left<<"\nRight Plain Text : "<<right<<endl;
    ...
    for(int i = 0; i < 16; i++) {
		...
		cout<<"Round "<<i + 1<<" "<<left<<" "<<right<<" "<<rkb[i]<<endl;
	}
...
int main() {
    ...
    cout<<"Enter plain text(in binary): ";
	cin>>text;
    ...
    string cipher = encrypt(text, rkb);
    ...
    string decryptedText = encrypt(cipher, rkb);
	
    cout<<"\nPlain Text: "<<decryptedText<<endl;
}
